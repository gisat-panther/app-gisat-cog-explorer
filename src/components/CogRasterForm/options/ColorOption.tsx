...
export default function ({ title, name, defaultValue, children }: { name: string, title: string, defaultValue: any, children: ReactNode }) {
	...
	return <label className="block">
		...
		<input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" onChange={onChanged} value={urlVal} type='text' placeholder={defaultValue} />
		{children}
	</label>
}